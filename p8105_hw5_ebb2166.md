...
Dataset and function creation with trial for 10 simulations
...
Simulation results trial for mu = 0
...
sim_results_df = 
  tibble(
    samp_mu = c(0, 1, 2, 3, 4, 5, 6)
  ) %>% 
  mutate(output_lists = map(.x = samp_mu, ~ rerun(5000, sim_ttest(mu = .x))),
